...
async function importTokensForMode(
  ...
  for (const [key, value] of Object.entries(tokens)) {
    // Skip internal properties (both formats)
    ...
    if (isTokenValue(value)) {
      // This is a leaf token - normalize to internal format
      const normalizedToken = normalizeToken(value, path);
      await createVariable(collection, modeId, path, normalizedToken);
...
function isTokenValue(obj: any): boolean {
  // Support W3C format ($value/$type), legacy format (value/type), and value-only format
  return obj && typeof obj === 'object' &&
    (('$value' in obj) || ('value' in obj && !hasNestedTokens(obj)));
}

function hasNestedTokens(obj: any): boolean {
  // Check if the object has nested token objects (not just value/type/description)
  for (const key of Object.keys(obj)) {
    if (key === 'value' || key === 'type' || key === 'description' ||
        key === '$value' || key === '$type' || key === '$description') {
      continue;
    }
    // If there's any other key, this might be a container, not a leaf token
    // Check if the nested value could be a token
...
function normalizeToken(obj: any, path: string = ''): TokenValue {
  // Convert W3C format ($value/$type) to internal format (value/type)
  // If type is not specified, infer from path
  const explicitType = obj.$type ?? obj.type;
  const inferredType = explicitType || inferTypeFromPath(path);
...
function inferTypeFromPath(path: string): string {
  ...
  // Check first segment and common patterns
  const firstSegment = segments[0];
  
  // Direct type mappings from path prefix
  const typeFromPrefix: Record<string, string> = {
    'color': 'color',
    'colors': 'color',
    'colours': 'color',
    'spacing': 'dimension',
  ...
  if (typeFromPrefix[firstSegment]) {
    // For font, check sub-type
    if (firstSegment === 'font' || firstSegment === 'typography') {
      ...
      if (lowerPath.includes('lineheight') || lowerPath.includes('line-height')) return 'number';
      return 'string';
    }
    return typeFromPrefix[firstSegment];
  }
  
  // Check for patterns anywhere in path
  if (lowerPath.includes('color') || lowerPath.includes('background') || 
      lowerPath.includes('foreground') || lowerPath.includes('border') ||
      lowerPath.includes('fill') || lowerPath.includes('stroke')) {
    return 'color';
  }
  
  if (lowerPath.includes('radius')) return 'dimension';
  if (lowerPath.includes('shadow')) return 'shadow';
