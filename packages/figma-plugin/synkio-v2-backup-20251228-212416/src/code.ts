...
async function handleSaveExcludedCollections(collections: string[]) {
  console.log('handleSaveExcludedCollections called with:', collections);
  ...
  console.log('Calling recalculateSyncStatus...');
  await recalculateSyncStatus();
  console.log('recalculateSyncStatus completed');
}
...
async function handleSaveExcludedStyleTypes(styleTypes: StyleType[]) {
  console.log('handleSaveExcludedStyleTypes called with:', styleTypes);
...
async function convertValueForFigma(value: unknown, type: string): Promise<VariableValue> {
  // Handle reference
...
async function recalculateSyncStatus() {
  console.log('recalculateSyncStatus: starting');
  const syncBaseline = loadChunked<BaselineData>(KEYS.SYNC_BASELINE);
  if (!syncBaseline) {
    // No baseline yet, nothing to recalculate
    console.log('recalculateSyncStatus: no syncBaseline, returning early');
    return;
  }
  ...
  console.log('recalculateSyncStatus: excludedCollections=', excludedCollections);
  console.log('recalculateSyncStatus: excludedStyleTypes=', excludedStyleTypes);
  ...
  console.log('recalculateSyncStatus: currentBaseline entries=', Object.keys(currentBaseline.baseline).length);
  console.log('recalculateSyncStatus: currentBaseline styles=', currentBaseline.styles ? Object.keys(currentBaseline.styles).length : 0);
  console.log('recalculateSyncStatus: syncBaseline entries=', Object.keys(syncBaseline.baseline).length);
  console.log('recalculateSyncStatus: syncBaseline styles=', syncBaseline.styles ? Object.keys(syncBaseline.styles).length : 0);
  ...
  console.log('recalculateSyncStatus: pendingChanges=', pendingChanges);
  console.log('recalculateSyncStatus: deletedVariables=', syncDiff.deletedVariables.length);
  console.log('recalculateSyncStatus: newVariables=', syncDiff.newVariables.length);
  ...
  console.log('recalculateSyncStatus: sending state-update with syncStatus=', syncStatus);
  sendMessage({
    type: 'state-update',
    state: {
      syncStatus,
...
function filterBaseline(
  ...
  // Filter out excluded collections from baseline
  const filteredBaseline: Record<string, BaselineEntry> = {};
  for (const [key, entry] of Object.entries(baseline.baseline)) {
    if (!excludedCollections.includes(entry.collection)) {
      filteredBaseline[key] = entry;
    }
  }

  // Filter out excluded style types from styles
  let filteredStyles: Record<string, StyleBaselineEntry> | undefined;
  if (baseline.styles) {
    filteredStyles = {};
    for (const [key, entry] of Object.entries(baseline.styles)) {
      if (!excludedStyleTypes.includes(entry.type)) {
        filteredStyles[key] = entry;
      }
    }
  }
