...
.docs-layout {
  display: block;
  min-height: 100vh;
  padding-left: calc(244px + var(--docs-space-lg) * 2);
}
...
.docs-sidebar {
  ...
  position: fixed;
  top: var(--docs-space-lg);
  left: var(--docs-space-lg);
  width: 244px;
  height: calc(100vh - var(--docs-space-lg) * 2);
  overflow-y: auto;
...
/* Mode toggle row - appears below header on pages with multiple modes */
.docs-mode-row {
  display: flex;
  align-items: center;
  gap: var(--docs-space-md);
...
.docs-mode-row--overview {
  flex-wrap: wrap;
  gap: var(--docs-space-lg);
}

.docs-collection-mode-toggle {
  display: flex;
  align-items: center;
  gap: var(--docs-space-sm);
}
...
.docs-config-title {
  ...
  font-weight: 600;
  color: var(--docs-text);
  margin: 0 0 var(--docs-space-md) 0;
